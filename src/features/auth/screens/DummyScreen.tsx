...
const getHeaderTabIcon = (label: string) => {
  ...
  if (normalizedLabel === 'all') {
    return {icon: 'shopping-outline', iconColor: '#5A3D00'};
  }
  ...
  if (normalizedLabel.includes('atta')) {
    return {icon: 'barley', iconColor: '#9b6200'};
  }
  ...
  return {icon: 'basket-outline', iconColor: '#7d1f4d'};
};
...
const PHONE_PRODUCT_ROW_SIZE = 2;
const TABLET_PRODUCT_ROW_SIZE = 3;
const INITIAL_RENDER_CONTENT_ITEMS = 10;
const CONTENT_PAGE_SIZE = 8;

const chunkProducts = (items: Product[], rowSize: number) => {
  const rows: Product[][] = [];
...
export default function DummyScreen() {
  ...
  const [featuredProducts, setFeaturedProducts] = useState<Product[]>(initialCachedProducts);
  const [heroHeaderHeight, setHeroHeaderHeight] = useState(0);
  const [isStickyHeaderActive, setIsStickyHeaderActive] = useState(false);
  const [isLoadingProducts, setIsLoadingProducts] = useState(
    initialCachedProducts.length === 0,
  );
  ...
  const productRowSize = isTablet
    ? TABLET_PRODUCT_ROW_SIZE
    : PHONE_PRODUCT_ROW_SIZE;
  const productGridWidth = `${100 / productRowSize - 2}%`;
  ...
  const headerTabs = React.useMemo(
    () => [
      { key: 'All', label: 'New', ...getHeaderTabIcon('All') },
  ...
  const contentItems = React.useMemo<HomeListItem[]>(() => {
    ...
    if (isAllHeaderTabSelected) {
      return displayedProductGroups.flatMap(group => {
        const productRows = chunkProducts(group.products, productRowSize).map(
          (products, index) => ({
  ...
  useEffect(() => {
    ...
    const loadProducts = async () => {
      ...
      if (isMounted && storedProducts.length > 0) {
        setFeaturedProducts(storedProducts);
  ...
  const heroHeaderAnimatedStyle = {
    opacity: scrollY.interpolate({
      inputRange: [0, Math.max(heroHeaderHeight * 0.45, 1), Math.max(heroHeaderHeight * 0.9, 1)],
    ...
    transform: [
      {
        translateY: scrollY.interpolate({
          inputRange: [0, Math.max(heroHeaderHeight, 1)],
          outputRange: [0, -18],
          extrapolate: 'clamp',
        }),
      },
  ...
  const renderHeroHeader = () => (
    <View
      onLayout={event => {
        ...
        if (nextHeight !== heroHeaderHeight) {
          setHeroHeaderHeight(nextHeight);
        }
      }}
      style={[
        styles.heroTop,
        {
          ...
          paddingTop: statusBarTopInset,
        },
      ]}
    >
      <Animated.View style={[styles.deliveryRow, heroHeaderAnimatedStyle]}>
        <Pressable
        ...
        >
          <View style={styles.metaRow}>
            <Text
              style={[
                styles.deliveryEta,
                {color: selectedHeaderTheme.titleText},
              ]}
            >
              Buy in{' '}
              <Animated.Text
                style={[
                  styles.deliveryEtaTime,
                  deliveryDigitAnimatedStyle,
              ...
              >
                ...
                <Text
                  ...
                  style={[
                    ...
                    isActive && {color: selectedHeaderTheme.titleText},
                  ]}
                >
                  {item.label}
                </Text>
                {isActive ? (
                  <View
                    style={[
                      ...
                      {backgroundColor: selectedHeaderTheme.titleText},
    ...
    <View style={styles.root}>
      ...
      <FlatList
        ...
        onScroll={event => {
          ...
          const stickyActivationOffset = Math.max(heroHeaderHeight - 12, 1);
          const stickyReleaseOffset = Math.max(heroHeaderHeight - 28, 0);

          setIsStickyHeaderActive(current =>
            current
              ? offsetY > stickyReleaseOffset
              : offsetY >= stickyActivationOffset,
          );
        }}
        ref={listRef}
            ...
            case 'productRow':
              return (
                ...
                >
                  ...
                  {Array.from({
                    length: Math.max(productRowSize - item.products.length, 0),
                  }).map((_, spacerIndex) => (
                    <View
                      key={`${item.key}-spacer-${spacerIndex}`}
                      style={[styles.productRowSpacer, {width: productGridWidth}]}
                    />
                  ))}
                </View>
              );
