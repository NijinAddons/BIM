...
const INFO_ITEMS: readonly InfoItem[] = [
  { label: 'Saved Addresses', icon: 'map-marker-outline', route: 'MyAddresses' },
...
type AlertIntent = 'logout' | 'delete-account' | null;

export default function ProfileScreen() {
  const insets = useSafeAreaInsets();
  const navigation = useNavigation<ProfileNavProp>();
  const [activeAlert, setActiveAlert] = useState<AlertIntent>(null);
  ...
  const onPressOtherInfoItem = (item: InfoItem) => {
    if (item.label === 'Logout') {
      setActiveAlert('logout');
    }

    if (item.label === 'Delete account') {
      setActiveAlert('delete-account');
    }
  };
  ...
  const alertConfig =
    activeAlert === 'logout'
      ? {
          message:
            'You will be signed out of Buy In Minutes on this device.',
  ...
  return (
    <View style={styles.container}>
      ...
      <CustomAlert
        ...
        onPrimaryButtonPress={alertConfig?.onPrimaryButtonPress}
        primaryButtonLabel={alertConfig?.primaryButtonLabel}
        primaryButtonVariant={alertConfig?.primaryButtonVariant}
